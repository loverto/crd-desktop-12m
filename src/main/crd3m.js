...
// 通过for循环构造模板坐标数组,总共12个坐标
// 1. 从左上到左下中间为6个坐标,
// 2. 然后从右下到右上中间为6个坐标,
// 3. 上下两个坐标的间距为65,
// 4. 左右两个坐标的间距为99,
// 5. 左上初始坐标为"508,225",备注,逗号左边的值为x坐标,右边的值为y坐标
// 坐标示例: let modelPosition = ["530,253","586,250","532,414","586,414"]
let modelPosition = []
// 生成左侧模板坐标
for(let i=0;i<6;i++){
    modelPosition[i] = "508,"+(225+i*65)
}
//获取最后一个坐标
...
// let modelPosition = ["508,225","591,235","532,414","586,414"]
            ...
            for (i; i<dataArr.length; i++){
                ...
                for (let j = 0;j<data.length;j++){
                    ...
                    if (fs.existsSync(picPath) && fs.existsSync(modelFilePath)){
                        handler(picPath,"mouldPath",false,modelPosition,picfilename,j,"modelNumber","keep")
                    }
